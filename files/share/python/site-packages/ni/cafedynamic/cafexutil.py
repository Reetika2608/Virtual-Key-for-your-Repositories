...
class CafeXUtils(object):
    ...
    def get_operation_status(connector_name, logger):
        ...
        try:
            ...
            if os.path.exists(status_path):
                ...
                json_status = CafeXUtils.read_json_file(status_path, logger)
    ...
    ######################################################
    #  Code required to de-couple ni src from FMC source #
    ######################################################
    @staticmethod
    def read_json_file(path, logger):
        ...
        try:
            if os.path.isfile(path):
                ...
                logger.debug('Detail="File does not exist: %s"' % path)
        except IOError:
            logger.debug('Detail="IO Error occured while reading %s"' % path)
        except ValueError:
            logger.debug('Detail="Invalid JSON at path = %s"' % path)
        return content
