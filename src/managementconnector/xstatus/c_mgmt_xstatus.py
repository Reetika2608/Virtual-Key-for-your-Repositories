...
def get_status(rest_cdb_adaptor):
    """
    ...
    try:
        ...
        # the name/value pair doesn't exist so exit with a known exit code, which will result in an empty list
...
def _get_status(entitled_connectors, _config, manifest_class):
    ...
    try:
        ...
        DEV_LOGGER.error('Detail="Error creating connector element: %r %s: %s"',
                         ex, ex.__str__(), traceback.format_exc())
        # an error occurred when generating the xml so exit with an "unknown" exit code
        # which will reraise the exception
        sys.exit(ManagementConnectorProperties.SYS_UNKNOWN_ERROR_CODE)

    return connectors_elem


def _process_return_output(status_element):
    """ process and return the xstatus output """
...
def main():
    """ xstatus invoked from shell script, or externally """
    _process_return_output(get_status(DatabaseHandler()))
