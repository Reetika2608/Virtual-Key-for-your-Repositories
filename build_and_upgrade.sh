...
usage(){
    ...
    echo "          -P   Port Number: sets the port number for your target. Default Port is 22"
...
add_to_known_hosts(){
    ...
    [[ ! -d ~/.ssh/ ]] && mkdir ~/.ssh/
    ssh-keyscan -p ${TARGET_PORT} -H ${TARGET} >> ~/.ssh/known_hosts
}

upgrade(){
    add_to_known_hosts
    # Set the version number to installed version + 1, if the -v option was not supplied
    INSTALLED_VERSION=$(sshpass -p ${TARGET_PASSWORD} ssh -p ${TARGET_PORT} root@${TARGET} dpkg  -l | grep c_mgmt | awk '{print $3}' | cut -d "." -f 4)
    if [[ -z "${VERSION}" ]]; then
...
install(){
    ...
    sshpass -p ${TARGET_PASSWORD} scp -P ${TARGET_PORT} ${pkg} root@${TARGET}:/tmp/pkgs/new/
...
uninstall(){
    ...
    sshpass -p ${TARGET_PASSWORD} ssh -p ${TARGET_PORT} root@${TARGET} 'echo c_mgmt > /tmp/pkgs/new/files.rem'
}
...
wait_for_install(){
    ...
    for i in `seq 1 45`;
    do
        installed_version=$(sshpass -p ${TARGET_PASSWORD} ssh -p ${TARGET_PORT} root@${TARGET} '(dpkg -s c_mgmt || true) | grep Version')

        if [[ ${installed_version} == *"${VERSION}" ]]; then
            install_status=$(sshpass -p ${TARGET_PASSWORD} ssh -p ${TARGET_PORT} root@${TARGET} 'dpkg -s c_mgmt | grep Status')
    ...
    echo $(sshpass -p ${TARGET_PASSWORD} ssh -p ${TARGET_PORT} root@${TARGET} 'dpkg -s c_mgmt')
    exit 1
}
...
TARGET_PASSWORD=x
TARGET_PORT=22
while getopts ":ht:p:v:c:n:P:w" opt; do
    case ${opt} in
        h )
          usage
          exit 0
