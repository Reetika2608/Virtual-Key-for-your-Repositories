...
import ni.diagnostics.alarmmanager
...
class MCAlarm():
    ...
    def __init__(self, config):
        """ constructor """
        self.alarm_manager = ni.diagnostics.alarmmanager.AlarmManager()
        self._config = config
            ...
            for alarm in alarm_list:
                if alarm['id'] not in excluded_alarms:
                    with self._alarm_lock:
                        try:
                            self.alarm_manager.lower_alarm(alarm['uuid'])
                        except IOError:
    ...
    def clear_alarm(self, guid):
        ...
        if self.is_raised(guid):
            ...
            with self._alarm_lock:
                self.alarm_manager.lower_alarm(guid)
    ...
    def raise_alarm(self, guid, params=None):
        ...
        with self._alarm_lock:
            if params is not None:
                self.alarm_manager.raise_alarm(guid, parameters=params)
            else:
                self.alarm_manager.raise_alarm(guid)
